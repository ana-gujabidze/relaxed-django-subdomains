...
def prefix_values(dictionary, prefix):
    return dict((key, "%s.%s" % (prefix, value)) for key, value in dictionary.items())
...
class SubdomainMiddlewareTestCase(SubdomainTestMixin, TestCase):
    ...
    def test_www_domain(self):
        ...
        with override_settings(REMOVE_WWW_FROM_DOMAIN=False):
            ...
            self.assertEqual(host("subdomain.www.%s" % self.DOMAIN), "subdomain")
            self.assertEqual(
                host("www.subdomain.www.%s" % self.DOMAIN), "www.subdomain"
            )
...
class SubdomainURLReverseTestCase(SubdomainTestMixin, TestCase):
    def test_url_join(self):
        ...
        self.assertEqual(
            urljoin(self.DOMAIN, scheme="https"), "https://%s" % self.DOMAIN
        )
    ...
    def test_explicit_reverse(self):
        ...
        self.assertEqual(
            reverse("home", subdomain="api"), "http://api.%s/" % self.DOMAIN
        )
        self.assertEqual(
            reverse("view", subdomain="api"), "http://api.%s/view/" % self.DOMAIN
        )
    ...
    def test_wildcard_reverse(self):
        ...
        subdomain = "wildcard"
        self.assertEqual(
            reverse("home", subdomain), "http://%s.%s/" % (subdomain, self.DOMAIN)
        )
        self.assertEqual(
            reverse("view", subdomain), "http://%s.%s/view/" % (subdomain, self.DOMAIN)
        )
    ...
    def test_using_not_default_urlconf(self):
        ...
        self.assertEqual(
            reverse("application", subdomain=subdomain),
